...
exports.run = (message, bot) => {
  ...
  if (allSeries[args]) {
    ...
    let nameList = pagedNames[0].map(content => content + "\n");
    let emb = new Discord.RichEmbed()
      .setTitle(message.args.toUpperCase())
      .setColor(0xe06666)
      .setDescription(
        `Series Type: ${rollList["metaData"][args]["type"]} \n ${
    ...
    message.channel.send(emb).then(msg => {
      if (series.names.length > maxNamePerPage) {
        ...
        collector.on("collect", e => {
          ...
          let nameList = pagedNames[index].map(content => content + "\n");
          let emb = new Discord.RichEmbed()
            .setTitle(message.args.toUpperCase())
            .setColor(0xe06666)
            .setDescription(
  ...
  if (allWaifu[args]) {
    ...
    if (dupList.hasOwnProperty(args)) {
      let waifus = dupList[args].map(
        waifu => `\n${waifu.name + " " + waifu.extraTag}`
      );
      let msg = `There is more than one character with the specified name: ${waifus
        .toString()
        .replace(/,/g, "")}`;
