...
let loadList = () => {
  ...
  fs.readdir("./Assets/Characters", function(err, files) {
    ...
    files.forEach(function(file) {
      let test = JSON.parse(
        fs.readFileSync("./Assets/Characters/" + file, "utf8")
      );
      ...
      test["characters"].forEach(character => {
        ...
        switch (test["type"]) {
          case "Anime":
            isFemale
              ? (animeWaifu[character.series] = series)
              : (animeHusbando[character.series] = series);
            break;
          case "Game":
            isFemale
              ? (vnGameWaifu[character.series] = series)
              : (vnGameHusbando[character.series] = series);
            break;
          case "Manga":
            isFemale
              ? (mangaWaifu[character.series] = series)
              : (mangaHusbando[character.series] = series);
            break;
          case "Kpop":
            isFemale
              ? (kpopWaifu[character.series] = series)
              : (kpopHusbando[character.series] = series);
            break;
        }
      });
    ...
    characterList.forEach(list => {
      Object.keys(list).forEach(seriesName => {
        if (ret.allSeries.hasOwnProperty(seriesName.toLowerCase())) {
          ...
          ret.allSeries[seriesName.toLowerCase()] = {
            names: list[seriesName].names
          };
        }
          ...
          if (ret.allWaifu.hasOwnProperty(waifuName.toLowerCase())) {
            if (metaData[seriesName.toLowerCase()].hasOwnProperty("extraTag")) {
              let newWaifuName =
                waifuName +
                " " +
                metaData[seriesName.toLowerCase()]["extraTag"];
              let existingWaifu = ret.allWaifu[waifuName.toLowerCase()];
              ret.allWaifu[
                waifuName.toLowerCase().trim() +
                  " " +
                  metaData[
                    existingWaifu.series.toLowerCase()
                  ].extraTag.toLowerCase()
              ] = existingWaifu;
              ret.allWaifu[newWaifuName.toLowerCase().trim()] =
                list[seriesName].datalist[waifuName];
            ...
            ret.allWaifu[waifuName.toLowerCase().trim()] =
              list[seriesName].datalist[waifuName];
          }
        });
      });
